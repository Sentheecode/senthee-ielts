...
export function CoachChat() {
  const [messages, setMessages] = useState<Message[]>([{ role: "agent", content: "你好。把错因或写作内容发来，我给你下一步。" }]);
  ...
  return (
    <div className="coach-chat">
      <div className="chat-status"><Bot aria-hidden="true" /><div><strong>Agent</strong><span>在线反馈 / 本地兜底</span></div></div>
      <div className="messages" aria-live="polite">{messages.map((message, index) => <div key={index} className={`message ${message.role}`}>{message.content}</div>)}{loading && <div className="message agent">正在看…</div>}</div>
